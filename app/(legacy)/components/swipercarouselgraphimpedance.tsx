    ...
    <>
      ...
      >
          {drawing && drawing.length > 0 && drawing.map((item, index) => (
            <SwiperSlide key={`${item.name} - ${index + 1}`} data-testid={`swiper-slide-drawing-${index + 1}`}>
          ...
          {graph && graph.length > 0 && graph.map((item, index) => (
            <SwiperSlide key={`${item.name} - ${index + 1}`} data-testid={`swiper-slide-graph-${index + 1}`}>
          ...
          {impedance && impedance.length > 0 && impedance.map((item, index) => (
            <SwiperSlide key={`${item.name} - ${index + 1}`} data-testid={`swiper-slide-impedance-${index + 1}`}>
                <div className="h-full flex justify-center items-center cursor-pointer"
                    ...
                    <Card className="border-none h-full w-full flex items-center justify-center bg-transparent hover:bg-slate-200">
                      <CardContent className="p-6 flex items-center justify-center w-full h-full">
                        <div className="relative overflow-hidden flex items-center justify-center h-[200px] w-full">
                          <LazyImageCustom
                            src={item.url.startsWith('/uploads/') ? `${process.env.NEXT_PUBLIC_ROOT_URL}${item.url}` : item.url} 
                            alt={`${item.name} - Impedance - ${index + 1}`} 
                            width={500}
                            height={500}
                            classname="max-h-full max-w-full object-contain"
                          />
         ...
         <>
          {repeated && drawing?.length > 0 &&
            drawing.map((item, index) => (
              <SwiperSlide key={`${item.name} - ${index + 1} - Drawing`}  data-testid={`swiper-slide-drawing-2-${index + 1}`}>
                <div
          ...
          {graph?.length > 0 &&
            graph.map((item, index) => (
              <SwiperSlide key={`${item.name} - ${index + 1} - Graph`}  data-testid={`swiper-slide-graph-2-${index + 1}`}>
                <div
                  className="h-full flex justify-center items-center cursor-pointer"
                  onClick={() =>
                    drawing?.length > 0
          ...
          {impedance?.length > 0 &&
            impedance.map((item, index) => {
              ...
              return (
                <SwiperSlide key={`${item.name} - ${index + 1} - Impedance`} data-testid={`swiper-slide-impedance-2-${index + 1}`}>
                  <div
                    className="h-full flex justify-center items-center cursor-pointer"
