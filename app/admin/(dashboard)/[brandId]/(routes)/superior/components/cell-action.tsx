...
import { SuperiorityColumn } from "./columns";
...
export const CellAction: React.FC<CellActionProps> = ({
  ...
  const onConfirm = async () => {
    try {
      ...
      const API_EDITED2 = API_EDITED.replace('{superiorId}', data.id)
      ...
      if(response.data === 'duplicate'){
        ...
        router.refresh();
        toast.success('Superiority deleted.');
      }
    } catch (error) {
      router.refresh()
  ...
  const onCopy = (id: string) => {
    ...
    toast.success('Superiority ID copied to clipboard.');
  }
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem
            onClick={() => router.push(`${process.env.NEXT_PUBLIC_ADMIN_FOLDER_URL}/${params.brandId}/superior/${data.id}`)}
          >
