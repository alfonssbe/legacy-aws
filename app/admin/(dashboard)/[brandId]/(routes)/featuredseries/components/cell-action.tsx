...
import { FeaturedSeriesColumn } from "./columns";
...
export const CellAction: React.FC<CellActionProps> = ({
  ...
  const onConfirm = async () => {
    try {
      ...
      const API_EDITED2 = API_EDITED.replace('{featuredSeriesId}', data.id)
      ...
      if(response.data === 'duplicate'){
        ...
        router.refresh();
        toast.success('Featured Series deleted.');
      }
    } catch (error) {
      router.refresh()
  ...
  const onCopy = (id: string) => {
    ...
    toast.success('Featured Series ID copied to clipboard.');
  }
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem
            onClick={() => router.push(`${process.env.NEXT_PUBLIC_ADMIN_FOLDER_URL}/${params.brandId}/featuredseries/${data.id}`)}
          >
